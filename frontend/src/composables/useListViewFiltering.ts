...
export function useListViewFiltering(
  ...
  const matchesRewatchFilter = (record: RecordType, filterEnabled: boolean, listId: number): boolean => {
    return !filterEnabled || listId !== 1 || matchesRewatchCriteria(record);
  };

  const matchesUnreleasedFilter = (record: RecordType, filterEnabled: boolean, listId: number): boolean => {
    return !filterEnabled || listId !== 2 || isMovieReleased(record);
  };

  const matchesRecentReleasesFilter = (record: RecordType, filterEnabled: boolean, listId: number): boolean => {
    return !filterEnabled || listId !== 2 || isRecentRelease(record.movie?.releaseDateTimestamp, 6);
  };
      ...
      return records.value.filter((record) => {
        try {
          ...
          return matchesSearchQuery(record, query) &&
                 matchesRewatchFilter(record, currentFilters?.toRewatch ?? false, listId) &&
                 matchesUnreleasedFilter(record, currentFilters?.hideUnreleased ?? false, listId) &&
                 matchesRecentReleasesFilter(record, currentFilters?.recentReleases ?? false, listId);
