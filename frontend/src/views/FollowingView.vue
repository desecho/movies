<template>
  <v-container>
    ...
    <v-row class="mb-6">
      <v-col cols="12">
        <div class="page-header">
          <h1 class="page-title">
            <v-icon icon="mdi-account-heart" class="title-icon" />
            {{ isPublicView ? `${username}'s Following` : "Following" }}
          </h1>
          <p class="page-subtitle">
            {{ isPublicView ? `People ${username} is following` : "People you're following" }}
          </p>
        </div>
      </v-col>
    </v-row>
    ...
    <div v-else-if="followingUsers.length === 0" class="empty-state">
      ...
      <h3>{{ isPublicView ? "Not following anyone yet" : "You're not following anyone yet" }}</h3>
      <p>
        {{
          isPublicView
            ? `${username} isn't following anyone yet.`
            : "Start following users to see their movie activity in your feed."
        }}
      </p>
    ...
    <div v-else>
      <div class="stats-header">
        <h3>{{ followingUsers.length }} following</h3>
      </div>
      ...
      <div class="users-grid">
        <div v-for="(user, index) in followingUsers" :key="index" class="user-card">
          <div class="user-card-content">
            ...
            <!-- Unfollow Button (only show for personal view) -->
            <div v-if="!isPublicView" class="unfollow-button-container">
              <v-btn
...
async function loadFollowingUsers(): Promise<void> {
  ...
  try {
    ...
    if (props.isPublicView && props.username) {
      // Load public following list for specified user
      apiUrl = getUrl(`users/${props.username}/following/`);
    } else {
      // Load personal following list
      if (!authStore.user.isLoggedIn) {
        $toast.error("Please log in to view your following list");
        return;
      }
      apiUrl = getUrl("user/following/");
    }
