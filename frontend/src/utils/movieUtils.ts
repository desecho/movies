 ...
 */
export const isValidRecord = (record: any): record is RecordType => {
  return record && 
         record.movie && 
 ...
 */
export const hasValidMovie = (record: RecordType): boolean => {
  return record.movie && 
         typeof record.movie.title === 'string' &&
 ...
 */
export const createSearchString = (record: RecordType): string => {
  if (!hasValidMovie(record)) return '';
 ...
 */
export const matchesSearchQuery = (record: RecordType, query: string): boolean => {
  if (!query.trim()) return true;
  if (!hasValidMovie(record)) return false;
 ...
 */
export const createDateFromTimestamp = (timestamp: unknown): Date | null => {
 ...
 */
export const isRecentRelease = (timestamp: unknown, monthsBack = 6): boolean => {
 ...
 */
export const getUserRating = (record: RecordType): number => {
 ...
 */
export const getImdbRating = (record: RecordType): number => {
 ...
 */
export const getCustomOrder = (record: RecordType): number => {
 ...
 */
export const getAdditionDate = (record: RecordType): number => {
 ...
 */
export const getReleaseDate = (record: RecordType): number => {
 ...
 */
export const isMovieReleased = (record: RecordType): boolean => {
 ...
 */
export const hasValidOptions = (record: RecordType): boolean => {
 ...
 */
export const matchesRewatchCriteria = (record: RecordType): boolean => {
  if (!hasValidOptions(record)) return false;
...
export const filterPredicates = {
  ...
  recentReleases: (listId: number, monthsBack = 6) => (record: RecordType): boolean => {
    return listId !== 2 || isRecentRelease(record.movie?.releaseDateTimestamp, monthsBack);
  }
};