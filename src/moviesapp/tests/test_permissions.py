...
class RecordOwnershipTestCase(BaseTestCase):
    ...
    def setUp(self):
        ...
        watched_list, _ = List.objects.get_or_create(
            key_name=f"watched_record_{unique_id}", defaults={"name": f"Watched Record {unique_id}", "id": watched_id}
        )

        self.own_record, _ = Record.objects.get_or_create(user=self.user, movie=movie, list=watched_list)
        ...
        self.other_record, _ = Record.objects.get_or_create(user=self.other_user, movie=other_movie, list=watched_list)
    ...
    def test_can_delete_own_record(self):
        ...
        test_record = Record.objects.create(user=self.user, movie=movie, list=watched_list)
...
class ListPermissionTestCase(BaseTestCase):
    ...
    def test_prevent_duplicate_records(self):
        ...
        Record.objects.get_or_create(user=self.user, movie=self.movie, list=self.watched_list)
...
class DataValidationTestCase(BaseTestCase):
    ...
    def setUp(self):
        ...
        self.record, _ = Record.objects.get_or_create(user=self.user, movie=movie, list=watched_list)
    ...
    def test_rating_bounds_validation(self):
        ...
        invalid_ratings = [-1, 11, 999]  # Remove -1 to avoid IntegrityError
    ...
    def test_sql_injection_prevention(self):
        ...
        malicious_usernames = ["'; DROP TABLE records; --", "admin' OR '1'='1", "' UNION SELECT * FROM users --"]
    ...
    def test_comment_length_handling(self):
        ...
        self.assertIn(
            response.status_code,
            [HTTPStatus.OK, HTTPStatus.BAD_REQUEST, HTTPStatus.UNAUTHORIZED, HTTPStatus.FORBIDDEN],
        )
